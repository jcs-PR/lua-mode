...
(defun lua--get-indentation-test-sections (file-path)
  (with-temp-buffer
	...
	;; XXX: Avoid error `Local variables entry is missing the suffix'
	;; on Windows with Emacs version lower than 29.
	(if (fboundp #'eask-ignore-errors)
		(eask-ignore-errors (hack-local-variables))
